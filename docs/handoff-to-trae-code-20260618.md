...
CheapLive 是浏览器端为主的应用。单设备面捕、程序化 Avatar 和 VoiceChanger 可在浏览器端独立运行。多设备 WebRTC 连接依赖本地局域网 Node.js HTTP/SSE 信令服务。
...
```
branch: main
本地 HEAD: b86508bbe6dbfb52fabe68133544b311c003192a
GitHub main HEAD: b86508bbe6dbfb52fabe68133544b311c003192a（已同步）

最近 commits:
b86508b feat: add debug avatar with facial parameter visualization
e4d4e8f feat: add browser-based face tracking prototype with MediaPipe
...

> ⚠️ 本文档之前的版本声称本地 HEAD 为 `d520376`，GitHub HEAD 为 `7ebd71a`，均已过时。
> 用户已于 2026-06-18 手动将 TRAE Work 的最新本地提交推送到 GitHub。
> 旧 commit 历史（含 d520376, 7ebd71a 等）已通过 force push 被重写，不再存在于 GitHub main。

**工作区状态**: 有未追踪文件（`artifacts/` 目录下的 webm 文件，为测试运行产物，非源码）。

**Push 状态**: GitHub 和 Gitee 均已推送完毕。
...
### Push 状态
- GitHub 和 Gitee 均已推送完毕（2026-06-18）
- 旧 commit 历史（d520376, 7ebd71a 等）已通过 force push 被重写
